...
def check_numeric(k, v):
    if not is_number(v):
        raise ValueError(
            'The value of "{}" is invalid. Expected input to be a number. Instead its type was {}.'.format(
                k, type(v)
            )
        )
...
def check_vector_2(k, v):
    if not np.size(v) == 2:
        raise ValueError(
            'The value of "{}" is invalid. Expected input to be a 2-element array.'.format(
                k
            )
        )
...
def check_int(k, v):
    if np.sum(np.array(v) % 1) != 0:
        raise ValueError(
            'The value of "{}" is invalid. Expected input to be an integer or list of integers.'.format(
                k
            )
        )
...
def check_fun(k, d, s):
    if k not in d.keys():
        raise ValueError(
            'I2MCfunc: "{}" must be specified using the "{}" option key, but it cannot be found'.format(
                s, k
            )
        )
...
def average_eyes(L_X, R_X, missing_x, L_Y, R_Y, missing_y):
    ...
    xpos[q] = (L_X[q] + R_X[q]) / 2.0
    ypos[q] = (L_Y[q] + R_Y[q]) / 2.0
...
def find_interp_wins(xpos, ypos, missing, window_time, edge_samples, freq, max_disp):
    ...
    window_samples = round(window_time / (1.0 / freq))
    ...
    for p in range(len(miss_start)):
        ...
        if (
            miss_start[p] < edge_samples + 1
            or (p > 0 and miss_end[p - 1] > miss_start[p] - edge_samples - 1)
            or miss_end[p] > len(xpos) - 1 - edge_samples
            or (
                p < len(miss_start) - 1
                and miss_start[p + 1] < miss_end[p] + edge_samples + 1
            )
        ):
            qRemove[p] = True
            continue
        ...
        for q in range(len(on)):
            ...
            displacement = np.hypot(
                np.nanmedian(xpos[resamps]) - np.nanmedian(xpos[lesamps]),
...
def windowed_interpolate(xpos, ypos, missing, miss_start, miss_end, edge_samples):
    ...
    for p in range(len(miss_start)):
        ...
        valid_samps = np.concatenate(
            (
                out_win[0] + np.arange(-edge_samples, 0),
...
def kmeans2(data):
    ...
    while True:
        ...
        if prevtotsumD <= totsumD:
            ...
            m = mlast
            iter -= 1
            # if m_has_two_entries(m):
            break
        if iter >= max_iterations:
            # if m_has_two_entries(m):
            break
            # else:
            # overstepped_iterations += 1

        # if overstepped_iterations >= max_iterations:
        #     print("ERROR: can't find an m, that has two entries...")
    ...
    # if m_has_two_entries(m):
    #     return label, C
    if not m_has_two_entries(m):
        # print("doesn't have two entries!!")
        return label, C
...
def two_cluster_weighting(
    ...
    num_samples = int(window_time / (1.0 / freq))
    step_size = np.max([1, int(step_time / (1.0 / freq))])
    ...
    if downsamp_filter:
        ...
        rp = 0.05  # passband ripple in dB
        b = [[] for i in range(nd)]
        a = [[] for i in range(nd)]
        for p in range(nd):
            b[p], a[p] = scipy.signal.cheby1(cheby_order, rp, 0.8 / downsamples[p])
    ...
    while eind <= (len(xpos) - 1):
        ...
        for p in range(nd + 1):
            ...
            switchesw[p] = 1.0 / np.sum(switches[p])
...
def get_fixations(final_weights, timestamp, xpos, ypos, missing, par):
    ...
    for p in range(1, len(starttime))[::-1]:
        ...
        if (
            starttime[p] - endtime[p - 1] < maxMergeTime
    ...
    for p in range(len(starttime)):
        if missing[fixstart[p]]:
            fixstart[p] = fixstart[p] + np.argmax(
                np.invert(missing[fixstart[p] : fixend[p] + 1])
            )
            starttime[p] = timestamp[fixstart[p]]
        if missing[fixend[p]]:
            fixend[p] = fixend[p] - (
                np.argmax(np.invert(missing[fixstart[p] : fixend[p] + 1][::-1])) + 1
            )
    ...
    nextSamp = np.min(
        np.vstack([fixend + 1, np.zeros(len(fixend), dtype=int) + len(timestamp) - 1]),
        axis=0,
    ...
    if not len(fixend) == 0 and fixend[-1] == len(
        timestamp
    ...
    for a in range(len(fixstart)):
        ...
        flankdataloss[a] = (fixstart[a] > 0 and missing[fixstart[a] - 1]) or (
            fixend[a] < len(xpos) - 1 and missing[fixend[a] + 1]
        )

        # fraction of data loss during fixation that has been (does not count
        # data that is still lost)
        fracinterped[a] = np.sum(np.invert(np.isnan(xposf[qMiss]))) / (
            fixend[a] - fixstart[a] + 1
        )
...
def get_fix_stats(xpos, ypos, missing, fix, pix_per_deg=None):
    ...
    for a in range(len(fstart)):
        ...
        if np.sum(qMiss) == len(qMiss):
            ...
            rangeX[a] = np.max(xposf[np.invert(qMiss)]) - np.min(
                xposf[np.invert(qMiss)]
            )
            rangeY[a] = np.max(yposf[np.invert(qMiss)]) - np.min(
                yposf[np.invert(qMiss)]
            )
...
def I2MC(gazeData, options=None, logging=True, logging_offset=""):
    ...
    par["chebyOrder"] = opt.pop(
        "chebyOrder", 8.0
    )  # order of cheby1 Chebyshev downsampling filter, default is normally ok, as long as there are 25 or more samples in the window (you may have less if your data is of low sampling rate or your window is small
    par["maxerrors"] = opt.pop(
        "maxerrors", 100.0
    )  # maximum number of errors allowed in k-means clustering procedure before proceeding to next file
    # FIXATION DETERMINATION
    par["cutoffstd"] = opt.pop(
        "cutoffstd", 2.0
    )  # number of standard deviations above mean k-means weights will be used as fixation cutoff
    par["onoffsetThresh"] = opt.pop(
        "onoffsetThresh", 3.0
    ...
    par["maxMergeDist"] = opt.pop(
        "maxMergeDist", 30.0
    )  # maximum Euclidean distance in pixels between fixations for merging
    par["maxMergeTime"] = opt.pop(
        "maxMergeTime", 30.0
    )  # maximum time in ms between fixations for merging
    par["minFixDur"] = opt.pop(
        "minFixDur", 40.0
    ...
    if par["downsampFilter"]:
        nSampRequired = (
            np.max([1, 3 * par["chebyOrder"]]) + 1
        )  # nSampRequired = max(1,3*(nfilt-1))+1, where nfilt = chebyOrder+1
        nSampInWin = round(par["windowtime"] / (1.0 / par["freq"]))
        if nSampInWin < nSampRequired:
            raise ValueError(
                'I2MC: Filter parameters requested with the setting "chebyOrder" '
    ...
    ), 'I2MCfunc: Some of your downsample levels are not divisors of your sampling frequency. Change the option "downsamples"'

    # setup visual angle conversion
    pix_per_deg = None
    if par["scrSz"] is not None and par["disttoscreen"] is not None:
        pix_per_deg = angle_to_pixels(
            1, par["disttoscreen"], par["scrSz"][0], [par["xres"], par["yres"]]
        )
    ...
    missStart, missEnd = find_interp_wins(
        xpos,
        ypos,
        missing,
        par["windowtimeInterp"],
        par["edgeSampInterp"],
        par["freq"],
        par["maxdisp"],
    )
    if q2Eyes:
        llmissStart, llmissEnd = find_interp_wins(
            data["L_X"].array,
            data["L_Y"].array,
            llmiss,
            par["windowtimeInterp"],
        ...
        rrmissStart, rrmissEnd = find_interp_wins(
            data["R_X"].array,
            data["R_Y"].array,
            rrmiss,
            par["windowtimeInterp"],
    ...
    if q2Eyes:
        llx, lly, llmissn = windowed_interpolate(
            data["L_X"].array,
            data["L_Y"].array,
            data["left_missing"].array,
            llmissStart,
            llmissEnd,
            par["edgeSampInterp"],
        )
        rrx, rry, rrmissn = windowed_interpolate(
            data["R_X"].array,
            data["R_Y"].array,
            data["right_missing"].array,
            rrmissStart,
            rrmissEnd,
            par["edgeSampInterp"],
        )
    ...
    if not q2Eyes:
        ...
        if logging:
            print(
                logging_offset + "I2MC: 2-Means clustering started for averaged signal"
            )
        data["finalweights"], stopped = two_cluster_weighting(
            xpos,
            ypos,
            missingn,
            par["downsamples"],
    ...
    ## CALCULATE 2-MEANS CLUSTERING FOR SEPARATE EYES
        ...
        if logging:
            print(
                logging_offset + "I2MC: 2-Means clustering started for left eye signal"
            )
        finalweights_left, stopped = two_cluster_weighting(
            llx,
            lly,
            llmissn,
            par["downsamples"],
        ...
        if logging:
            print(
                logging_offset + "I2MC: 2-Means clustering started for right eye signal"
            )
        finalweights_right, stopped = two_cluster_weighting(
            rrx,
            rry,
            rrmissn,
            par["downsamples"],
    ...
    if logging:
        print(
            logging_offset
            + "I2MC: Determining fixations based on clustering weight mean for averaged signal and separate eyes + {:.2f}*std".format(
                par["cutoffstd"]
            )
        )
    fix = get_fixations(
        data["finalweights"].array, data["time"].array, xpos, ypos, missing, par
    )
...
#### from I2MC_example.py


def get_variables(
    screen_res, screen_size, screen_distance, sampling_rate, missing_value, **kwargs
):
    ...
    opt["yres"] = float(screen_res[1])  # maximum value of vertical resolution in pixels
    opt[
        "missingx"
    ] = missing_value  # missing value for horizontal position in eye-tracking data (example data uses -xres). used throughout the algorithm as signal for data loss
    opt[
        "missingy"
    ] = missing_value  # missing value for vertical position in eye-tracking data (example data uses -yres). used throughout algorithm as signal for data loss
    opt[
        "freq"
    ] = sampling_rate  # sampling frequency of data (check that this value matches with values actually obtained from measurement!)
    ...
    # instead of degrees.
    opt["scrSz"] = screen_size  # screen size in cm
    opt["disttoscreen"] = screen_distance  # distance to screen in cm.
    ...
    log_level = 0  # 1    # 0: no output, 1: output from this script only, 2: provide some output on command line regarding I2MC internal progress
    do_plot_data = False  # True # if set to True, plot of fixation detection for each trial will be saved as png-file in output folder.
    ...
    opt[
        "windowtimeInterp"
    ] = 0.1  # max duration (s) of missing values for interpolation to occur
    opt[
        "edgeSampInterp"
    ] = 2  # amount of data (number of samples) at edges needed for interpolation
    opt["maxdisp"] = (
        opt["xres"] * 0.2 * np.sqrt(2)
    )  # maximum displacement during missing for interpolation to be possible

    # # K-MEANS CLUSTERING
    opt[
        "windowtime"
    ] = 0.2  # time window (s) over which to calculate 2-means clustering (choose value so that max. 1 saccade can occur)
    opt[
        "steptime"
    ] = 0.02  # time window shift (s) for each iteration. Use zero for sample by sample processing
    opt[
        "maxerrors"
    ] = 100  # maximum number of errors allowed in k-means clustering procedure before proceeding to next file
    opt["downsamples"] = [2, 5, 10]
    opt[
        "downsampFilter"
    ] = False  # use chebychev filter when downsampling? Its what matlab's downsampling functions do, but could cause trouble (ringing) with the hard edges in eye-movement data

    # # FIXATION DETERMINATION
    opt[
        "cutoffstd"
    ] = 2.0  # number of standard deviations above mean k-means weights will be used as fixation cutoff
    opt[
        "onoffsetThresh"
    ] = 3.0  # number of MAD away from median fixation duration. Will be used to walk forward at fixation starts and backward at fixation ends to refine their placement and stop algorithm from eating into saccades
    opt["maxMergeDist"] = (
        kwargs.get("maxMergeDist") or 30.0
    )  # maximum Euclidean distance in pixels between fixations for merging
    opt["maxMergeTime"] = (
        kwargs.get("maxMergeTime") or 30.0
    )  # maximum time in ms between fixations for merging
    opt["minFixDur"] = (
        kwargs.get("minFixDur") or 40.0
    )  # minimum fixation duration after merging, fixations with shorter duration are removed from output
    return opt, log_level


##### add-on for eyeflow-lab plugin


def convert_elab_to_data(df_raw, time_unit, **kwargs):

    # always seem to use one eye, but if binocular, already used binoc as gaze_x and gaze_y!
    rename_dict = {"gaze_x": "L_X", "gaze_y": "L_Y"}

    data = df_raw.rename(rename_dict, axis=1)

    for col in rename_dict.values():
        data[col] = data[col].astype(float)

    if time_unit == "micro":
        data["time"] = data["time"].astype(float) / 1000

    return data


def convert_fix_to_elab(fix, df_raw, eye, time_unit, **kwargs):
    ...
    for new_col, val in val_dict.items():
        df_fix[new_col] = val
    ...
    rename_dict = {
        ...
        "orig_fix_x": "xpos",
        "orig_fix_y": "ypos",
        "disp_x": "fixRangeX",  # "RMSxy",
        "disp_y": "fixRangeY",  # "RMSxy"
    }

    for new_col, old_col in rename_dict.items():
        df_fix[new_col] = df_fix[old_col]

    df_fix = df_fix[list(rename_dict.keys()) + list(val_dict.keys())]

    df_fix.reset_index(inplace=True, drop=True)
    df_fix["id"] = df_fix.index
    df_fix["stimfile"] = df_raw.stimfile.iloc[0]
    df_fix["sid"] = df_raw.sid.iloc[0]
    df_fix["trialid"] = df_raw.trialid.iloc[0]

    return df_fix


def detect_events(df_raw, eye, progress_cb, task, time_unit="micro", **kwargs):

    Logger = logging.getLogger(df_raw.sid.iloc[0])
    data = convert_elab_to_data(df_raw, time_unit, **kwargs)
    opt, log_level = get_variables(**kwargs)

    Logger.info(f"Detect Fixations for task {task} with options: {opt}")
    fix, _, _ = I2MC(data, opt, logging=False)

    Logger.info(f"Real Mean Sampling Rate: {1000/np.mean(np.diff(data.time))}")
    Logger.info(f"Detected N(fix): {len(fix['start'])}")

    df_fix = convert_fix_to_elab(fix, df_raw, eye, time_unit, **kwargs)

    return {"FIXATIONS": df_fix}


INFO = {"events": ["FIXATIONS"]}
...
if __name__ == "__main__":
    ...
    data["time"] = data["t_et"]
    # df_raw = pd.read_csv("raw_test2.csv")
    # df_raw["time"] = df_raw["time"]/1000
    # df_raw["R_X"] = df_raw["gaze_x_right"]
    # df_raw["R_Y"] = df_raw["gaze_y_right"]
    # data = df_raw[["time", "R_X", "R_Y"]]
